...
public class TiledBackground : MonoBehaviour
{
    ...
    // create a public field where we can determine whether objects this script is applied to can be scaled horizontally and/or vertically, as applied below
    ...
    void Start()
    {
        var newWidth = !scaleHorizontally ? 1 : Mathf.Ceil(Screen.width / (textureSize * PixelPerfectCamera.scale)); // use of Ceil prevents rounding down so that we cannot possibly be left with a value that will leave untiled gaps at the edge of screen
        var newHeight = !scaleVertically ? 1 : Mathf.Ceil(Screen.height / (textureSize * PixelPerfectCamera.scale)); // PixelPerfectCamera.scale is the ratio of the screen size to the native resolution. This is used since te original size of the texture is being amplified when scaled up to meet screen size.
